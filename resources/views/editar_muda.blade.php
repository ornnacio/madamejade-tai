...
<html>
    ...
    <body>
		...
		<div style="width: 60%; margin: 0 auto; justify-content: center">
			<form method='POST' action="{{ route('editar_muda', ['id' => $muda->id]) }}">
				@csrf
				<div class="form-group">
					<input class="form-control" name="espécie" placeholder="Espécie" value="{{ $muda->espécie }}" required>
				</div>
				<div class="form-group">
					<div class="row">
						<div class="col">
							<label>Data de Plantação</label>
							<input type="date" class="form-control" name="data_plantação" value="{{ $muda->data_plantação }}" required>
						</div>
						<div class="col">
							<label>Data de Germinação</label>
							<input type="date" class="form-control" name="data_germinação" value="{{ $muda->data_germinação }}" required>
						</div>
					</div>
				</div>
				<div class="form-group">
					<input class="form-control" name="número_filhas" placeholder="Número de filhas" value="{{ $muda->número_filhas }}" required>
				</div>
