...
<html>
    ...
    <body>
		...
		<div style="width: 60%; margin: 0 auto; justify-content: center">
			<form method='POST' action="{{ route('editar_pedido', ['id' => $pedido->id]) }}">
				@csrf
				<div class="form-group">
					<input class="form-control" name="cliente" placeholder="Cliente" value="{{ $pedido->cliente }}" required>
				</div>
				<div class="form-group">
					<textarea class="form-control" name="desc" placeholder="Descrição" rows="3" required>{{ $pedido->descrição }}</textarea>
				</div>
				<div class="form-group">
					<input class="form-control" name="total" placeholder="Total" value="{{ $pedido->total }}" required>
				</div>
				<div class="form-group">
					<select class="form-control" name="id_produto" required>
						<?php foreach($produtos as $p): ?>
							<option value="<?=$p->id?>" <?php if($p->id === $pedido->id_produto) echo "selected"; ?>><?=$p->nome?></option>
						<?php endforeach; ?>
					</select>
				</div>
