...
<html>
    ...
    <body>
		...
		<div class="container-fluid">
			<form method="POST" action="{{ route('buscar_muda') }}">
				@csrf
				<div class="row" style="margin: auto">
					...
					<div class="col">
						<div class="float-right d-flex flex-row">
							<div class="form-outline">
								<a class='btn btn-primary' href="{{ url('/adicionar_muda') }}">
			...
			<table class="table table-striped">
				<thead>
					<tr>
						...
						<th scope="col">Data de plantação</th>
						<th scope="col">Data de germinação</th>
						<th scope="col">Quantidade de filhas</th>
						<th scope="col">Editar</th>
						<th scope="col">Deletar</th>
					</tr>
				</thead>
				<tbody>
					<?php foreach($mudas as $m): ?>
						<tr>
							<th><?= $m->id; ?></th>
							<td><?= $m->espécie; ?></td>
							<td><?= $m->data_plantação; ?></td>
							<td><?= $m->data_germinação ?></td>
							<td><?= $m->número_filhas; ?></td>
							<td><a style="font-size: 25px;" href="{{ route('editar_muda', ['id' => $m->id]) }}"><i class="fas fa-edit"></i></a></td>
							<td><a style="font-size: 25px; color: red;" href="{{ route('deletar_muda', ['id' => $m->id]) }}" onclick="return confirm('Deletar esse item?');" ><i class="far fa-trash-alt"></i></a></td>
						</tr>
					<?php endforeach; ?>
				</tbody>
			</table>
