...
class LoginController extends Controller
{
	...
	public function store(Request $rq){
		...
		if($senha == $confSenha){
			...
			if(count($checkEmail) > 0){
				return redirect('cadastro')->with('msgErro', 'Email já cadastrado');
			}
			...
			if($created){
				return redirect('login')->with('msgSucesso', 'Registrado com sucesso!');
			}
		}else{
			return redirect('cadastro')->with('msgErro', 'Confirmação e senha diferem');
		}
	...
	public function verificar(Request $rq){
		...
		if(count($session) > 0){
			...
			$rq->session()->forget('msgSucesso');
			return redirect('login')->with('msgErro', 'Email ou senha inválidos');
		}
